import random
import time
def board(pName,pc,p):
    print("\t\t\t  :  " + pName.upper() + "   vs   "+pc )
...
if __name__ == '__main__':
    again=1
    print('\n\n\n!!@saurabhsaha027-------------: PlAY  ("_") TIC TAC TOE ("_")  WITH COMPUTER/Friend :-----------!!')
    print(" `````````````````````````````````````````````````````````````````````````````")
    print("---------: MODE :---------\n 1 - COMPUTER :\n 2 - FRIEND:\n ")
    ...
    while(again):
        while (cnt < 9):
            if (cnt % 2 == 0):
                ...
                choice = input("Turns Enter Choice :")
                choice = ord(choice) - 48
                if (choice > 0 and choice < 10):
                    if (p[choice - 1] == cs or p[choice - 1] == ps):
                        print("------>Invalid Choice<-----")
        ...
        if (cnt > 8):
            print("   ----->>-->  No-One Wons ,<--<<-------")
        print("Wanna play again: type 'yes': Otherwise click any key Except Enter")
        sk=input().upper()
        if(sk!="YES"):
            again=0
            print("\t\t\t\t\t---->>>-->>>>--THANKU YOU--<<<<<<<<----------------")
